...
- The credits page uses `PageHeader`, teal `text-link`s and a responsive grid of equal-size 48px avatars. Contributors and supporters share the same avatar-above-name cell so names wrap instead of truncating. Avatars come from one shared manifest with hashed PNG/WebP sheets served from the same origin and are rendered as decorative `aria-hidden` CSS sprites; members without a sheet get a neutral filled circle with the poi character cut out (a two-layer CSS mask using the derived `poi-character-mask` alpha). Special-thanks icon slots have transparent backgrounds. Thank-you copy and the contribution and Open Collective links sit beside each section heading and stay visible in every state. No cards, stats or donor leaderboards.
