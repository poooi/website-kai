...
- The credits page uses `PageHeader`, teal `text-link`s and a responsive grid of equal-size avatars with explicit dimensions and `loading="lazy"`. Contributors and supporters share the same avatar-above-name cell so names wrap instead of truncating. Thank-you copy and the contribution and Open Collective links sit beside each section heading and stay visible in every state; supporters come from Open Collective’s all-members document and are keyed by `MemberId`. No cards, stats or donor leaderboards.
