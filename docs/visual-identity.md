...
- Narrow and tablet navigation collapses into a native `<details>` disclosure with a localized “Menu” summary. It works without JavaScript, closes on link navigation, route changes and Escape (returning focus to the summary), keeps `aria-current="page"` on the active link, and never scrolls horizontally. Desktop keeps the inline nav, including Credits.
- The credits page uses `PageHeader`, teal `text-link`s and a responsive grid of equal-size 48px avatars. Contributors and supporters share the same avatar-above-name cell so names wrap instead of truncating. Avatars come from one shared manifest with hashed PNG/WebP sheets served from the same origin and are rendered as decorative `aria-hidden` CSS sprites; members without a sheet get a 48px name-initial circle whose muted HSL hue is deterministically derived from the NFC-normalized source name (light and dark variants; blank names show `?`). These avatar identity hues are separate from the existing action and link colors, which remain unchanged and must not be reused. Special-thanks icon slots have transparent backgrounds. Thank-you copy and the contribution and Open Collective links sit beside each section heading and stay visible in every state. No cards, stats or donor leaderboards.
