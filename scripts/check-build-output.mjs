...
const forbiddenWorkerPatterns = [
  ...
  {
    label: 'Next.js runtime import marker',
    pattern: /next\/(?:server|navigation|headers|og)/,
  },
]
...
const forbiddenClientPatterns = [
  ...
  {
    label: 'Node.js client import marker',
...
if (!existsSync(wranglerConfigPath)) {
  ...
  if (wranglerConfig.assets?.directory !== '../client') {
    fail(
      `Expected assets.directory ../client, got ${wranglerConfig.assets?.directory}`,
    )
  }
...
if (sourceMaps.length > 0) {
  fail(
    `Production build emitted ${sourceMaps.length} sourcemaps without Sentry upload`,
  )
}

const clientJs = clientFiles.filter((file) => file.endsWith('.js'))
const serverJs = serverFiles.filter((file) => file.endsWith('.js'))
const clientEntry = clientJs.find((file) =>
...
if (!clientEntry) {
  ...
  assertBudget(
    'Client entry chunk',
    (await readFile(clientEntry)).byteLength,
    budgets.maxClientEntryKiB,
  )
}
...
if (!existsSync(serverEntry)) {
  ...
  assertBudget(
    'Server entry chunk',
    (await readFile(serverEntry)).byteLength,
    budgets.maxServerEntryKiB,
  )
}

const clientJsTotal = (
  await Promise.all(
    clientJs.map(async (file) => (await readFile(file)).byteLength),
  )
).reduce((sum, size) => sum + size, 0)
assertBudget(
  'Total client JavaScript',
  clientJsTotal,
  budgets.maxClientJsTotalKiB,
)
